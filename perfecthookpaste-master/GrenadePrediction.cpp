...
void grenade_prediction::Paint()
{
    ...
    if ((type) && path.size()>1)
    {
        ...
        for (auto it = path.begin(), end = path.end(); it != end; ++it)
        {
            ...
            {
				if (g_Options.Misc.grenadecolors == 1)
				{
					g_Surface->DrawSetColor(255, 0, 0, 255);

				}

				if (g_Options.Misc.grenadecolors == 2)
				{
					g_Surface->DrawSetColor(200, 200, 200, 255);

				}

				if (g_Options.Misc.grenadecolors == 3)
				{
					g_Surface->DrawSetColor(0, 0, 255, 255);

				}

				if (g_Options.Misc.grenadecolors == 4)
				{
					g_Surface->DrawSetColor(255, 0, 255, 255);

				}

				if (g_Options.Misc.grenadecolors == 5)
				{
					g_Surface->DrawSetColor(255, 255, 0, 255);

				}

				if (g_Options.Misc.grenadecolors == 6)
				{
